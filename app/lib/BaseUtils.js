    ...
    BaseUtils = {
      ...
      resetIframe: function (dialog) {
        ...
        try {
          if (dialog) {
            ...
            dialog.reset();
          } else if (window.detailDialog && window.detailDialog.height) {
            window.detailDialog.height(height);
      ...
      dialog: function (options) {
        ...
        if (typeof options.load === 'function') {
          options.oniframeload = options.load;
        }
        seajs.use(['dialog'], function (dialog) {
          window[options.id ||
            'dialog'] = dialog(options).show(options.target);
        });
      },
      /**
       * 初始化复制按钮
