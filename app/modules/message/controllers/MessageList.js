...
define('MessageList', ['jquery', 'MessageModel', 'BaseCollection', 'BaseItem', 'BaseList', 'HandlebarsHelper',
    'template/message_list', 'template/message_item', 'BaseUtils', 'template/message_search', 'template/message_transfer',
    'template/message_sort'],
  function (require, exports, module) {
    var MessageModel, BaseCollection, BaseItem, BaseList, HandlebarsHelper, MessageList, MessageItem,
      MessageCollection, listTemp, itemTemp, searchTemp, BaseUtils, transferTemp, sortTemp;
    ...
    searchTemp = require('template/message_search');
    BaseUtils = require('BaseUtils');
    transferTemp = require('template/message_transfer');
    sortTemp = require('template/message_sort');
    ...
    MessageItem = BaseItem.extend({
      ...
      // 初始化
      initialize: function () {
        if (!app.getData('messageCategory')) {
          BaseUtils.getMessageCategory({
            extend: true,
            select: true
          }).then(function (list) {
            app.setData('messageCategory', list);
          })
        }
        this.model.set('messageCategoryList', app.getData('messageCategory'));
      ...
      // 上移
      moveUp: function () {
        app.getView('messageList')._moveUp(this.model);
      },
      // 下移
      moveDown: function () {
        app.getView('messageList')._moveDown(this.model);
      }
    });
    /**
     * 列表视图
        ...
        seajs.use(['dialog-plus'], function (dialog) {
          ...
          ctx.searchDialog = dialog({
            ...
            content: ctx.searchTemp({
              messageCategoryList: app.getData('messageCategory'),
              ...
              button: [
                {
                  ...
                  callback: function () {
                    ...
                    $.ajax({
                      type: 'POST',
                      async: false,
                      url: CONST.API + '/message/batch/transfer',
                      data: {
      ...
      batchDisplay: function () {
        ...
        if (this.checkboxIds = this._getCheckboxIds()) {
          $.ajax({
            type: 'POST',
            async: false,
            url: CONST.API + '/message/batch/display',
            data: {
            ...
            button: [
              {
                ...
                callback: function () {
                  ...
                  $.ajax({
                    type: 'POST',
                    async: false,
                    url: CONST.DOMAIN + '/user_v2/message',
                    data: {
